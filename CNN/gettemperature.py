...
def fetch_hourly_temperature(url, start_datetime_str, end_datetime_str):
    ...
    while current_datetime <= end_datetime:
        ...
        if response.status_code == 200:
            data = response.json()
            # Assuming your API and JSON structure, you might need to adjust the processing logic here
            # Example of processing data
            temperature = data.get('features', [{}])[0].get('properties', {}).get('measures', {}).get('temperature',
                                                                                                      {}).get('value',
                                                                                                              'N/A')
            print(f"DateTime: {current_datetime}, Temperature: {temperature}°C")
        else:
